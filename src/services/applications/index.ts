    ...
    const decryptedData = data
      ? {
          ...
          position: data.position
            ? await encryptionService.decrypt(data.position)
            : null,
          contact: data.contact
            ? await encryptionService.decrypt(data.contact)
            : null,
        }
      : undefined;
...
export async function updateApplication(
  ...
  try {
    ...
    if (updates.company) {
      encryptedUpdates.company = await encryptionService.encrypt(
        updates.company
      );
    }
    if (updates.position) {
      encryptedUpdates.position = await encryptionService.encrypt(
        updates.position
      );
    }
    if (updates.status) {
    ...
    if (updates.contact) {
      encryptedUpdates.contact = await encryptionService.encrypt(
        updates.contact
      );
    }
...
export async function getApplications(
  ...
  try {
    ...
    if (!emailThreads || (emailThreads && !emailThreads.length)) {
      return { success: true, data: [] };
    }
    // Debug log
    ...
    const testDecrypt = await encryptionService.decrypt(
    ...
    const decryptedApplications = await Promise.all(
      applications.map(async (app) => {
        const decryptedApp = await encryptionService.decryptApplicationData(
          app
        );
        return {
          ...app,
    ...
    const decryptedThreads = await Promise.all(
      (emailThreads ?? []).map(async (thread) =>
        encryptionService.decryptProcessedEmail(thread)
      )
    );
    ...
    const finalData = decryptedApplications.map((app) => ({
      ...app,
      email_thread: decryptedThreads.filter(
        (thread) => thread.application_id === app.id
      ),
    }));
