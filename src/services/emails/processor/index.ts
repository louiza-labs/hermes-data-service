...
export async function processAndStoreEmails(userId: string, emails: any[]) {
  ...
  for (const email of emails) {
    try {
      ...
      // Extract company name from email domain (simple fallback without AI)
      ...
      const companyName =
        extractCompanyFromEmail(email.senderEmail) ||
        extractCompanyFromEmail(email.receiverEmail) ||
        "Unknown Company";
      const company = { companyName, confidence: 0.5 };

      // Default status without AI
      const statusResult = { status: "applied", confidence: 0.5 };

      // Try to extract job role from subject (simple extraction)
      const extractJobRoleFromSubject = (subject: string): string => {
        const commonRoles = [
          "engineer",
          "developer",
          "manager",
      ...
      const jobRole = {
        jobRole: extractJobRoleFromSubject(email.subject),
        confidence: 0.5,
      };

      if (!company?.companyName || company.companyName === "Unknown Company") {
        console.log(`No company name found for email`);
        continue;
      }

      // Determine the company contact (from company domain)
      const companyDomain = company.companyName
        .toLowerCase()
        .replace(/[^a-z0-9]/g, "");
      const contact = email.senderEmail.toLowerCase().includes(companyDomain)
        ? email.senderEmail
        : email.receiverEmail;
      ...
      if (existingApplications) {
        for (const app of existingApplications) {
          const decryptedAppCompany = await encryptionService.decrypt(
            app.company
          );
          if (
            decryptedAppCompany?.toLowerCase().trim() ===
        ...
        if (!applicationId) {
          for (const app of existingApplications) {
            ...
            if (
              appCompany?.includes(emailCompany) ||
              emailCompany.includes(appCompany)
            ) {
              applicationId = app.id;
      ...
      const encryptedEmail = {
        ...
        receiverEmail:
          (await encryptionService.encrypt(email.receiverEmail)) || "",
        date: email.date,
        threadId: (await encryptionService.encrypt(email.threadId)) || "",
        snippet:
          (await encryptionService.encrypt(email.snippet || "")) || "" || "",
      };
      ...
      if (!applicationId) {
        ...
        const result = await createApplicationWithEmail(
          ...
          {
            companyName:
              (await encryptionService.encrypt(company.companyName)) || "",
          },
          statusResult.status || "applied",
          {
            jobRole:
              (await encryptionService.encrypt(jobRole?.jobRole || "")) || "",
          },
        ...
        const { error: emailError } = await supabase
          ...
          .insert({
            ...
            email_sender_email: await encryptionService.encrypt(
              email.senderEmail
            ),
            email_receiver_email: await encryptionService.encrypt(
              email.receiverEmail
            ),
            date: email.date,
