...
export const getGmailEmails = async (
  ...
  try {
    ...
    const allEmailResults = await Promise.all(
      tokensToProcess.map(async (tokenData: any) => {
        try {
          ...
          const currentToken = freshTokens.find(
          ...
          for (const message of messages) {
            ...
            const emailTypeResult = determineEmailTypeWithoutAI(
              subject,
              body,
              senderEmail || "",
              receiverEmail || ""
          ...
          console.error(
            `Error processing email account ${tokenData.email}:`,
            error
          );
    ...
    const nextPageToken = allEmailResults.find(
